...
public class CoachService {
        ...
        public LatestCoachSessionResponse generateDailyCoaching(
                ...
                CoachSession yesterdaySession = coachSessionRepository.findTopByOrderByCreatedAtDesc()
                                .filter(session -> !session.getCreatedAt().toLocalDate()
                                                .equals(java.time.LocalDate.now()))
                                .orElse(null);

                if (yesterdaySession != null && yesterdaySession.getActionItems() != null
                                && !yesterdaySession.getActionItems().isEmpty()) {
                        long completedCount = yesterdaySession.getActionItems().stream()
                                        .filter(ActionItem::getCompleted).count();
                        int completionPercent = (int) ((completedCount * 100)
                                        / yesterdaySession.getActionItems().size());

                        if (completionPercent < 50) {
                                systemPrompt += "\n\nCRITICAL CONTEXT: The user completed only " + completionPercent
                                                + "% of yesterday's tasks. YOU MUST reduce today's scope to prevent overload. Prioritize consistency over volume. Simplify the plan.";
                ...
                if (request.workspacePath() != null && !request.workspacePath().trim().isEmpty()) {
                        try {
                                ...
                                if (java.nio.file.Files.exists(startPath)
                                                && java.nio.file.Files.isDirectory(startPath)) {
                                        ...
                                        try (java.util.stream.Stream<java.nio.file.Path> stream = java.nio.file.Files
                                                ...
                                                stream.filter(p -> {
                                                        ...
                                                        return !name.contains("node_modules") && !name.contains(".git")
                                                                ...
                                                                .sorted((p1, p2) -> {
                                                                        try {
                                                                                return java.nio.file.Files
                                                                                                .getLastModifiedTime(p2)
                                                                                                .compareTo(java.nio.file.Files
                                                                                                                .getLastModifiedTime(
                                                                                                                                p1));
                                                                        } catch (Exception e) {
                                                                                return 0;
                                                                ...
                                                                .forEach(p -> {
                                                                        try {
                                                                                ...
                                                                                contextBuilder.append(content
                                                                                                .length() > 1000 ? content.substring(0, 1000) + "...(truncated)" : content);
                                                                        } catch (Exception e) {
                                                                                // ignore read errors
                                                                        }
                                                                });
                ...
                try {
                        ...
                        DailyCoachResponse parsed = objectMapper.readValue(
                                        response,
                        ...
                        return null; // For MVP, if it fails, the frontend handles it or we could throw an exception
                                     // instead of returning a hardcoded response, but let's just throw for cleaner
                                     // error handling or return null.
                }
        }
        ...
        private LatestCoachSessionResponse mapToDto(CoachSession session) {
                java.util.List<ActionItemDto> actionItemDtos = session.getActionItems().stream()
                                .map(item -> new ActionItemDto(item.getId(), item.getDescription(),
                                                item.getCompleted()))
                                .toList();
